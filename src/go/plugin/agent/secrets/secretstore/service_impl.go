...
func (s *inMemoryService) Normalize(cfg Config) (Config, error) {
	prepared, err := s.prepareConfig(context.Background(), cfg)
	if err != nil {
		return nil, err
	}
	return cloneConfig(prepared.rawConfig), nil
}
...
func (s *inMemoryService) Update(key string, cfg Config) error {
	...
	if before.configJSON == prepared.configJSON {
		return nil
	}
...
func (s *inMemoryService) prepareConfig(ctx context.Context, cfg Config) (preparedStore, error) {
	...
	payload := raw.ProviderPayload()
	if payload == nil {
		payload = map[string]any{}
	}
	bs, err := json.Marshal(payload)
	if err != nil {
		return preparedStore{}, fmt.Errorf("store '%s': marshaling provider payload: %w", key, err)
	}
	...
	canonicalRaw, err := canonicalizeConfig(raw, store.Configuration())
	if err != nil {
		return preparedStore{}, fmt.Errorf("store '%s': canonicalizing provider config: %w", key, err)
	}
	raw = canonicalRaw
	key = raw.ExposedKey()
	kind = raw.Kind()
	...
	configJSON := string(raw.DataJSON())
