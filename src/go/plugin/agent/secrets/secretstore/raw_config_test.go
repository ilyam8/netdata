...
func TestConfigDataJSONExcludesInternalMetadata(t *testing.T) {
	cfg := Config{
		"name":            "prod",
		"kind":            string(KindVault),
		"__source__":      "user.yaml",
	...
	require.NoError(t, json.Unmarshal(cfg.DataJSON(), &out))

	assert.Equal(t, "prod", out["name"])
	assert.Equal(t, string(KindVault), out["kind"])
	assert.Contains(t, out, "auth")
	assert.NotContains(t, out, "__source__")
	assert.NotContains(t, out, "__source_type__")
}

func TestConfigProviderPayloadStripsExternalMetadataAndClones(t *testing.T) {
	cfg := Config{
		"name":            "prod",
		"kind":            string(KindVault),
		"__source__":      "user.yaml",
	...
	assert.NotContains(t, payload, "kind")
	assert.NotContains(t, payload, "__source__")
	assert.NotContains(t, payload, "__source_type__")
	assert.Contains(t, payload, "auth")

	auth := payload["auth"].(map[string]any)
	auth["mode"] = "changed"
	selectors := auth["selectors"].([]any)
	selectors[1].(map[string]any)["alt"] = "CHANGED"

	origAuth := cfg["auth"].(map[string]any)
	assert.Equal(t, "token", origAuth["mode"])
	assert.Equal(t, "VAULT_TOKEN_ALT", origAuth["selectors"].([]any)[1].(map[string]any)["alt"])
}
