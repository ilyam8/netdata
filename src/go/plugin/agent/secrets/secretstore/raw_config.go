...
// from content hashing and JSON readback.
...
func (c Config) DataJSON() []byte {
	bs, _ := json.Marshal(c.dataMap(func(key string) bool {
...
func (c Config) ProviderPayload() map[string]any {
	return c.dataMap(func(key string) bool {
		return !strings.HasPrefix(key, "__") && key != keyName && key != keyKind
	})
}
...
func (c Config) dataMap(include func(key string) bool) map[string]any {
	if len(c) == 0 {
		return nil
	}
	...
	for k, v := range c {
		...
		switch t := v.(type) {
		case map[string]any:
			data[k] = cloneAnyMap(t)
		case []any:
			data[k] = cloneAnySlice(t)
		default:
			data[k] = t
		}
	}
