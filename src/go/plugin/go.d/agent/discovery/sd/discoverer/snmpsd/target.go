...
func newTargetGroup(sub subnet) *targetGroup {
	return &targetGroup{
		...
		source:   fmt.Sprintf("discoverer=snmp,network=%s", sub.str),
	}
}
...
func newTarget(ip string, credential configSnmpCredentials, si SysInfo) *target {
	tg := &target{
		IPAddress:  ip,
	...
	tg.hash, _ = hashstructure.Hash(target{}, nil)
...
type (
	target struct {
		...
		IPAddress  string
		Credential configSnmpCredentials `hash:"ignore"`
